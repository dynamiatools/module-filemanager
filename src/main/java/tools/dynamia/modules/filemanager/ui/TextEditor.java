...
public class TextEditor extends Window implements ActionEventBuilder {
    ...
    private void init() {
        ...
        contentbox.setDynamicProperty("onkeydown", "if(event.keyCode===9){var v=this.value,s=this.selectionStart,e=this.selectionEnd;this.value=v.substring(0, s)+'\\t'+v.substring(e);this.selectionStart=this.selectionEnd=s+1;return false;}");
    ...
    private void readFile() {
        try {
            String content = Files.readAllLines(file.toPath(), Charset.forName("UTF-8")).stream().collect(Collectors.joining("\n"));
